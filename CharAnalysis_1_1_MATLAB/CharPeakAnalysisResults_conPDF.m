...
%       plot(x,prctile(y,99)*0.78*y5(:,1),'.k','color',[.5 .5 .5])
%       plot(x,prctile(y,99)*0.85*y5(:,2),'.k','color',[.5 .5 .5])
%       plot(x,prctile(y,99)*0.92*y5(:,3),'.k','color',[.5 .5 .5])
...
%     plot(x,prctile(y,99)*mIndex*y5(:,end),'.k','color',[1 1 1])
%     plot(x,prctile(y,99)*mIndex*y5(:,end),'+k')    
...
subplot(3,5,11:13)  % Local signal-to-noise index
    ...
    'out','XTick',[0:1000:max(zoneDiv)],'XTickLabel',[0:max(zoneDiv)/1000],'YTick',...
    [0.1:0.1:1],'YTIckLabel',[{'0.1' '' '' '' '0.5' '' '' '' '' '1.0'}])
    box off
    xlabel ('time (cal. yr BP x 1000)','FontSize',FS)
...
subplot(3,5,14)     % Global signal-to-noise index
    ...
    'xtick',[1],'xticklabel','','ylim',y_lim,'YTick',[0.1:0.1:1],...
    'YTIckLabel',[{'0.1' '' '' '' '0.5' '' '' ''...
    '' '1.0'}],'yscale','log')
    box off; grid off
    y_lim = ylim; ylim ([y_lim(1) 1.01]) 
    title('(d) Global signal-to-noise index','FontSize',FS,'FontWeight',FW)
    xlabel([{'global signal-to-'}; {'noise distribution'}],'FontSize',FS)
    ylabel ('')
    if median(CharThresh.SNI) < 0.1
        sig_text = ['0.0',...
            num2str(round(median(CharThresh.SNI)*100))];
    else
        sig_text = ['0.',...
            num2str(round(median(CharThresh.SNI)*100))];
    end
...
for i = 1:length(zoneDiv)-1
    ...
    FRIBinKS = [0:20:5000]; % Bins for FRIs for KS test.
        % Add a warning is FRIs are longer than FRIBinKS
    wbl_cdf = wblcdf(FRIBinKS,param(1),param(2));    % Estimated wbl cdf.
    [h,pKS,t] = kstest(FRI,[FRIBinKS', wbl_cdf']); % resutls of test
...
subplot(3,5,1:4); hold off  % Peak Magnitude.
    ...
    title ([{'Peak magnitude, FRIs, smoothed fire frequ.'},...
        {''},{char(site)},{''}],...
        'FontSize',FS,'FontWeight','Bold','VerticalAlignment','Bottom');

subplot(3,5,9); % Smoothed fire frequency. 12/2008: added confidence-
    % interval estimates on smoothed fire frequency. Currenlty (12/08)
    % turned off (% below). - PEH
    hold off
%     nBoot = 100;
%     alpha = 0.10;
...
subplot(3,5,11:14); hold off    % FRIs through time.
    peak_yrs = Charcoal.ybpI(find(CharcoalCharPeaks(:,end) > 0)); % Years
        % with charocal peaks.
    if length(peak_yrs) > 2
    yPlot = diff(peak_yrs); % [yr] FRIs
    x = peak_yrs(1:end-1);  % [ybp] years for each corrsponding FRI in y
    x_lim = [0 1.1*max(yPlot)];
    plot(x,yPlot,'sk','MarkerFaceColor',[.75 .75 .75],'MarkerSize',4)
    set(gca,'FontSize',FS,'XDir','reverse','XMinorTick','on',...
        'YMinorTick','on','TickDir','out','XTick',[0:1000:max(zoneDiv)],...
        'Ylim',x_lim,'XTickLabel',[],'XLim',[zoneDiv(1),...
        zoneDiv(length(zoneDiv))],'box','off','position',...
        [0.1 .6 wm*range(zoneDiv) .12],'yScale','linear')
    if length(Charcoal.ybpI(Charcoal.ybpI<max(x))) > 0; % Resampled years.
        xi = Charcoal.ybpI(Charcoal.ybpI<max(x)); % Resampled years.
    else
        xi = 0;
    end
    if length(yPlot) > 1            % If there is more than 1 FRI...
        yi = interp1(x,yPlot,xi);   % Resampled FRIs.
    else
        yi = zeros(size(yPlot));    % If there is 1 or less FRI, fill with 
            % zeros. 
    end
    yis = smooth(yi, PeakAnalysis.peakFrequ/r,'lowess');   % Smoothed yi.
    hold on
    plot(xi,yis,'k','linewidth',1)    % Plot smoothed FRIs.
    ...
    ylabel ('FRI (yr fire^-^1)','FontSize',FS)
    else
    yis = NaN;
end
...
%         csvwrite(char([fileName(1:end-15) '_charResults.csv']),charResults,1,0);
    end
end