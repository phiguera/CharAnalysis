...
%     - All smooth() calls replaced by charLowess().
...
%     - NaN values in Charcoal.peak replaced with neutral values (0 for
%       residuals, 1 for ratios) before distribution fitting, preserving
%       window size near record gaps.
%     - Bug fix: GMM poor-fit fallback now passes X (local window), not
%       Charcoal.peak (full record).
...
for i = 1:nPk
    ...
    % Replace NaN values with the noise-component mean before distribution
    % fitting. NaN entries come from record gaps in Charcoal.peak.
    % Stripping them changes window size and distribution shape near gaps,
    % producing different GMM fits than v1.1. Replacing with the neutral
    % value (0 for residuals, 1 for ratios) preserves window size while
    % keeping gap samples from influencing the fit.
    ...
    % If fewer than 4 non-neutral samples exist, fall back to simple Gaussian.
    neutralVal = double(PeakAnalysis.cPeak ~= 1);  % 0 for residuals, 1 for ratios
    if sum(X ~= neutralVal) < 4
        if PeakAnalysis.cPeak == 1
            negVals = X(X <= 0);
            if isempty(negVals)
                sigmaHat(i,1) = std(X);
