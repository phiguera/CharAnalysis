...
% z_Compare_CharAnalysis_V1_V2   Regression test for CharAnalysis v2.0.
%   z_Compare_CharAnalysis_V1_V2(paramsFile, referenceFile)
%   z_Compare_CharAnalysis_V1_V2(paramsFile, referenceFile, 'Name', Value, ...)
%
...
%     'MagTol'      : maximum allowed total magnitude pct difference (default 2.0)
%     'FreqTol'     : maximum allowed mean fire frequency difference (default 0.001)
%     'Verbose'     : true/false, print per-sample details (default false)
%
%   EXAMPLE
%     z_Compare_CharAnalysis_V1_V2('CO_charParams.csv', 'CO_charResults_V_1_1.csv')
%     z_Compare_CharAnalysis_V1_V2('CH10_charParams.csv', 'CH10_charResults_V_1_1.csv', ...
%         'PeakTol', 1, 'ThreshTol', 0.015, 'FreqTol', 0.2)
%
...
addParameter(p, 'FreqTol',   0.001, @isnumeric);
...
if ~exist(v20File, 'file')
    ...
    candidates = candidates(~strcmpi({candidates.name}, [refName '.csv']));
...
pass    = pctDiff <= magTol;
if pass; nPass = nPass+1; else; nFail = nFail+1; end
fprintf('Total magnitude:  v1.1 = %.2f,  v2.0 = %.2f,  diff = %.1f pct  %s\n', ...
...
end % z_Compare_CharAnalysis_V1_V2
