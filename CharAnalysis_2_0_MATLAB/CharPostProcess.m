...
% Expand scalar SNI to full vector for global threshold path.
% CharThreshLocal returns SNI as [N x 1]; CharThreshGlobal returns a scalar.
if isscalar(CharThresh.SNI)
    SNI_col = CharThresh.SNI * ones(N, 1);
else
    SNI_col = CharThresh.SNI;
end
...
charResults(:, 1:22) = [Charcoal.cmI,  Charcoal.ybpI,  Charcoal.countI, ...
    ...
    SNI_col,  CharThresh.GOF(:,1), CharcoalCharPeaks, ...
    Charcoal.peakInsig, Charcoal.peakMagnitude, Charcoal.smoothedFireFrequ];if ~isnan(yis)
