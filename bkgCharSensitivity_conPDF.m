...
if max(Charcoal.ybpI) >= 1000
    ...
    bkgMax = n(find(x));    % Find largest value for background window.
end
...
[x,n] = hist(bkgMin,bkgBin);
    bkgMin = n(find(x));    % Find smallest value for background window.
end
...
subplot(2,3,1)
boxplot(GOF_i')
set(gca,'XTickLabel',bkSmooth,'TickDir','out')
ylim ([0 1])
...
subplot(2,3,2)
boxplot(SNI_i')
set(gca,'XTickLabel',bkSmooth,'yscale','log','YTick',[0.1:0.1:1.0],...
    'YTickLabel',[0.1:0.1:1.0],'TickDir','out')
ylim ([0.5 1])
xlabel ('smoothing window width (yr)')
